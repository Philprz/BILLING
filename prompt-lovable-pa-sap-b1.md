...
4. **Validation → SAP B1**
   - Bouton **« Valider et intégrer »** → choix *Facture de service* ou *Écriture au journal* (par défaut = préférence stockée sur le fournisseur ou globale).
...
- KPIs en haut : *À traiter*, *Prêtes*, *Intégrées aujourd'hui*, *En erreur*, *Rejetées*.
...
- **Gauche (50 %)** : viewer PDF de la facture reçue (pdf.js), onglets pour *PDF*, *XML source*, *Pièces jointes*.
- **Droite (50 %)** : panneau d'édition en 4 blocs :
  ...
  2. **Lignes** : tableau éditable — description, quantité, PU, HT, code TVA SAP, TVA, TTC, **compte comptable** (combobox liée au plan comptable SAP B1, affiche `AcctCode — AcctName`), centre de coût.
     ...
     - Actions par ligne : *Accepter la suggestion*, *Modifier*, *Créer une règle*.
  3. **Récapitulatif TVA** : par taux, contrôle cohérence total PA vs total calculé (alerte si écart > 0,01 €).
  4. **Intégration SAP** : radio *Facture de service* / *Écriture au journal*, sélecteur série (Series), date de comptabilisation (TaxDate / DocDate), référence (NumAtCard = n° PA), commentaire. Boutons : **Valider et intégrer** (primary), **Rejeter** (destructive), **Enregistrer le brouillon**.
